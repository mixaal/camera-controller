...
int
camera_eosviewfinder(GPParams *ctx, int onoff) 
{
...
main(int argc, char **argv) 
{
	...
	retval = gp_camera_init(ctx.camera, ctx.context);
	if (retval != GP_OK) {
		printf("  Retval: %d\n", retval);
		exit (1);
	}
	//canon_enable_capture(&ctx, TRUE);
	retval = camera_eosviewfinder(&ctx, 1);
	if (retval != GP_OK) {
	...
	retval = camera_eosviewfinder(&ctx,0);
	if (retval != GP_OK) {
